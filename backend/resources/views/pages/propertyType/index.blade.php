    ...
    <div class="px-10 mt-8 w-full">
        <div class="flex justify-end mt-10">
            <a href="{{ route('propertyType.create') }}"
                class="px-2 py-1 rounded-md bg-blue-500 text-sky-100 hover:bg-blue-700">+
                Create</a>
        </div>
        @if ($status)
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="bg-white">
                            @foreach ($propertyTypes as $propertyType)
                                <tr>
                                    <td class="px-6 whitespace-no-wrap border-b border-gray-200">{{ $propertyType->id }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        {{ $propertyType->name }}
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        <form action="{{ route('propertyType.destroy', $propertyType->id) }}"
                                            method="POST">
                                            <a href="{{ route('propertyType.edit', $propertyType->id) }}"
